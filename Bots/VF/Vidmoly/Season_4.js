...
const DB_NAME = 'C:\\Users\\Alex19\\Desktop\\Program\\Clear\\Data\\Anime Links\\VF\\Season 4\\Vidmoly\\Season_4.db';
const JSON_FILE = 'C:\\Users\\Alex19\\Desktop\\Program\\Clear\\Data\\Anime Links\\VF\\Season 4\\Vidmoly\\Season_4.json';
...
io.on('connection', (socket) => {
    ...
    socket.on('start', async (data) => {
        ...
        log(`🚀 Démarrage du Bot (Mode : Saisons 4+ Uniquement)`, 'info');
...
async function runBot(startFromName = null) {
    ...
    // -------------------------------------------------------------------------
    // PHASE 2 : TRAITEMENT (FILTRE SAISON 4)
    // -------------------------------------------------------------------------
    ...
    for (const link of allAnimeLinks) {
        ...
        // =====================================================================
        // VÉRIFICATION D'EXISTENCE DE LA SAISON 4 + vidmoly
        // =====================================================================
        let hasValidSeason4 = false;
        log(`🔍 Vérification présence Saison 4 + vidmoly...`, 'info');

        for (const checkLang of CONFIG.LANGUES) {
            if (!isRunning) break;
            const s4Url = `${cleanLink}/saison4/${checkLang}/`;

            try {
                await page.goto(s4Url, { waitUntil: 'domcontentloaded' });
        ...
        if (!hasValidSeason4) {
            log(`⏩ Anime ignoré (Pas de Saison 4 ou pas de vidmoly).`, 'skip');
            continue;
        }

        log(`✅ Saison 4 validée. Démarrage...`, 'success');

        // =====================================================================
        // BOUCLE DE SCRAPING (Commence à s=4)
        // =====================================================================
        let stopAnime = false;

        for (let s = 4; s <= CONFIG.MAX_SAISONS_CHECK; s++) {
            ...
            if ((!seasonFound && s > 4) || stopAnime) break;
        }
    }
...
server.listen(3100, () => {
    console.log('✅ Serveur lancé sur http://localhost:3100');
});